...
module.exports = function(app) {
  ...
  app.get('/monthitems/:monthid', function(req, res){
    var monthid = req.params.monthid;
        // Uses Mongoose schema to run the search (empty conditions)
        var name = req.query.asd
        console.log(name)

        var query = Items.find({"monthId":monthid});
        query.exec(function(err, monthlydetails){
            if(err)
                res.send(err);

            // If no errors are found, it responds with a JSON of all users
            res.json({ success: true, message: '',content:monthlydetails });
        });
    });
...
*/
    ...
    app.post('/item', function(req, res){
        ...
        newitem.save(function(err){
          ...
          message: 'Error in saving Items',content:err});

            // If no errors are found, it responds with a JSON of the new user
            
             res.json({
          success: true,
          message: 'Items are saved Sucessfully',
        });
        });
    });
    ...
    app.delete('/deleteitem', function(req, res){
        ...
        query.exec(function(err, item){
            if(err)
                res.json({success: false,
          message: 'Error in deleting Items',content:err});

            // If no errors are found, it responds with a JSON of all users
            res.json({ success: true, message: 'Item deleted',content:item });
        });
    });
    ...
    app.put('/updateitem', function(req, res){
        ...
        query.exec(function(err, item){
            if(err)
                res.json({success: false,
          message: 'Error in Updating Items',content:err});

            // If no errors are found, it responds with a JSON of all users
            res.json({ success: true, message: ' Item updated',content:item });
        });
